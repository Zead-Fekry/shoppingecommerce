...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    SystemChrome.setSystemUIOverlayStyle(SystemUiOverlayStyle(
      statusBarColor: Colors.orange, // Change color of status bar
                                    ...
                                    children: [
                                      ...
                                      Padding(
                                        padding: const EdgeInsets.only(
                                            top: 1.0, right: 18.0, left: 18.0,bottom: 10),
                                        child: Text(
  ...
  Widget  buildHomeAllProducts(context) {
    if (_isAllProductInitial && _allProductList.length == 0) {
      ...
      return GridView.builder(
        ...
        padding: EdgeInsets.only(left: 16.0,bottom: 1,top: 1, right: 16),
  ...
  Widget buildHomeAllProducts2(context) {
    if (_isAllProductInitial && _allProductList.length == 0) {
      ...
      return GridView.builder(
          ...
          itemCount: _allProductList.length,
          shrinkWrap: true,
          padding: EdgeInsets.only(top: 10.0, bottom: 5, left: 18, right: 18),
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, index) {
            return LayoutBuilder(
              builder: (context,constraints) {
                return ProductCard(
                    ...
                    has_discount: _allProductList[index].has_discount,
                  discount: _allProductList[index].discount,
                  cardHeight: constraints.minHeight,
                );
              }
            );
          });
...
Widget  buildHomeCarouselSlider(context) {
    if (_isCarouselInitial && _carouselImageList.length == 0) {
      return Padding(
          padding:
              const EdgeInsets.only(left: 1, right: 1, top: 0, bottom: 20),
        ...
        items: _carouselImageList.map((i) {
          return Builder(
            builder: (BuildContext context) {
              return Padding(
                padding: const EdgeInsets.only(
                    left: 1, right: 1, top: 12, bottom: 20),
                child: Stack(
                  children: <Widget>[
                    Container(
                      //color: Colors.amber,
